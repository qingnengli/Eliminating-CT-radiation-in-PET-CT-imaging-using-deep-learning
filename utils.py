...
def read_image_with_augment(path):
    ...
    image = tf.image.decode_jpeg(image, FLAGS.img_ch)
...
def LR_schedule(iterations, init_lr, type='Piecewise'):
    ...
    else: raise ValueError(type +' is not pre-defined.')
    return lr_schedule
...
def NRMSE(x,y, keep_dims = False):
    ...
    if keep_dims: return nrmse
    else: return tf.reduce_mean(nrmse)
...
def PSNR(x,y,keep_dims=False):
    ...
    if keep_dims: return psnr
    else: return tf.reduce_mean(psnr)

def SSIM(x,y, keep_dims=False):
    tensor_x = scale_to_minmax(x)
    tensor_y = scale_to_minmax(y)
    ssim = tf.image.ssim(tensor_x, tensor_y, 1.0)
    if keep_dims: return ssim
    else: return tf.reduce_mean(ssim)
...
def PCC(x, y, keep_dims=False):
    ...
    if keep_dims: return r
    else: return tf.reduce_mean(r)
